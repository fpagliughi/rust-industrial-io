...
pub struct Device {
    ...
    dev: NonNull<ffi::iio_device>,
    /// The IIO context containing the device.
    ctx: Context,
}

impl Device {
    /// Create a new device in the context
    pub(crate) fn new(dev: *mut ffi::iio_device, ctx: Context) -> Option<Self> {
        NonNull::new(dev).map(|dev| Self { dev, ctx })
    }

    /// Gets a pointer to the C device object
    pub(crate) fn as_ptr(&self) -> *mut ffi::iio_device {
        self.dev.as_ptr()
    }
    ...
    pub fn id(&self) -> Option<String> {
        let pstr = unsafe { ffi::iio_device_get_id(self.as_ptr()) };
        cstring_opt(pstr)
    }

    /// Gets the name of the device
    pub fn name(&self) -> Option<String> {
        let pstr = unsafe { ffi::iio_device_get_name(self.as_ptr()) };
        cstring_opt(pstr)
    }
    ...
    pub fn label(&self) -> Option<String> {
        let pstr = unsafe { ffi::iio_device_get_label(self.as_ptr()) };
        cstring_opt(pstr)
    }
    ...
    pub fn has_attrs(&self) -> bool {
        unsafe { ffi::iio_device_get_attrs_count(self.as_ptr()) > 0 }
    }

    /// Gets the number of device-specific attributes
    pub fn num_attrs(&self) -> usize {
        unsafe { ffi::iio_device_get_attrs_count(self.as_ptr()) as usize }
    }

    /// Gets the name of the device-specific attribute at the index
    pub fn get_attr(&self, idx: usize) -> Result<String> {
        let pstr = unsafe { ffi::iio_device_get_attr(self.as_ptr(), idx as c_uint) };
    ...
    pub fn find_attr(&self, name: &str) -> Option<String> {
        ...
        let pstr = unsafe { ffi::iio_device_find_attr(self.as_ptr(), cname.as_ptr()) };
        cstring_opt(pstr)
    }
    ...
    pub fn attr_read_str(&self, attr: &str) -> Result<String> {
        ...
        let ret = unsafe {
            ffi::iio_device_attr_read(self.as_ptr(), attr.as_ptr(), buf.as_mut_ptr(), buf.len())
        };
    ...
    pub fn attr_read_bool(&self, attr: &str) -> Result<bool> {
        ...
        let attr = CString::new(attr)?;
        let ret = unsafe { ffi::iio_device_attr_read_bool(self.as_ptr(), attr.as_ptr(), &mut val) };
        sys_result(ret, val)
    }
    ...
    pub fn attr_write_str(&self, attr: &str, val: &str) -> Result<()> {
        ...
        let ret = unsafe { ffi::iio_device_attr_write(self.as_ptr(), attr.as_ptr(), val.as_ptr()) };
        sys_result(ret as i32, ())
    }
    ...
    pub fn attr_write_bool(&self, attr: &str, val: bool) -> Result<()> {
        let attr = CString::new(attr)?;
        let ret = unsafe { ffi::iio_device_attr_write_bool(self.as_ptr(), attr.as_ptr(), val) };
        sys_result(ret, ())
    }
    ...
    pub fn attr_write_int(&self, attr: &str, val: i64) -> Result<()> {
        let attr = CString::new(attr)?;
        let ret = unsafe { ffi::iio_device_attr_write_longlong(self.as_ptr(), attr.as_ptr(), val) };
        sys_result(ret, ())
    }
    ...
    pub fn attr_write_float(&self, attr: &str, val: f64) -> Result<()> {
        let attr = CString::new(attr)?;
        let ret = unsafe { ffi::iio_device_attr_write_double(self.as_ptr(), attr.as_ptr(), val) };
        sys_result(ret, ())
    }
    ...
    pub fn num_channels(&self) -> usize {
        unsafe { ffi::iio_device_get_channels_count(self.as_ptr()) as usize }
    }

    /// Gets a channel by index
    pub fn get_channel(&self, idx: usize) -> Result<Channel> {
        let chan = unsafe { ffi::iio_device_get_channel(self.as_ptr(), idx as c_uint) };
    ...
    pub fn create_buffer(&self, sample_count: usize, cyclic: bool) -> Result<Buffer> {
        Buffer::new(self.clone(), sample_count, cyclic)
    }
    ...
    pub fn sample_size(&self) -> Result<usize> {
        let ret = unsafe { ffi::iio_device_get_sample_size(self.as_ptr()) };
    ...
    pub fn reg_read(&self, addr: u32) -> Result<u32> {
        let mut val: u32 = 0;
        let ret = unsafe { ffi::iio_device_reg_read(self.as_ptr(), addr, &mut val) };
        sys_result(ret as i32, val)
    }

    /// Sets the value of a hardware register
    pub fn reg_write(&self, addr: u32, val: u32) -> Result<()> {
        let ret = unsafe { ffi::iio_device_reg_write(self.as_ptr(), addr, val) };
        sys_result(ret as i32, ())
    }
}
